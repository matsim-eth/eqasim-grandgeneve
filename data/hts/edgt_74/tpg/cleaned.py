...
"""
This stage cleans the Annemasse EDGT (TPG version).
"""

def configure(context):
    context.stage("data.hts.edgt_74.tpg.raw")
...
def execute(context):
    df_households, df_persons, df_trips, df_spatial = context.stage("data.hts.edgt_74.tpg.raw")
    ...
    # Weight
    df_persons["person_weight"]       = df_persons["POND_PERS"].astype(float)
    df_households["household_weight"] = df_households["POND_MEN"].astype(float)
    ...
    df_trips = pd.merge(df_trips, df_spatial.rename(columns = {
        "ZFM": "LIEU_DEPART_FR", "departement_id": "origin_departement_id"
    }), on = "LIEU_DEPART_FR", how = "left")

    df_trips = pd.merge(df_trips, df_spatial.rename(columns = {
        "ZFM": "LIEU_ARRIVEE_FR", "departement_id": "destination_departement_id"
    }), on = "LIEU_ARRIVEE_FR", how = "left")
    ...
    # Clean employment
    df_persons["employed"] = df_persons["STATUT_TRAVAIL"].isin(["1", "2"])

    # Studies
    df_persons["studies"] = df_persons["STATUT_TRAVAIL"].isin(["3", "4", "5"])

    # Number of vehicles
    df_households["number_of_vehicles"] = df_households["NB_AUTO"] + df_households["NB_2RM"]
    ...
    # License
    df_persons["has_license"] = df_persons["PERMIS_CONDUIRE"] == "1"

    # Has subscription
    df_persons["has_pt_subscription"] = df_persons["ABO_TC"].isin(["1", "2", "3", "5", "6"])
    ...
    df_persons["DEPL_OUI_NON"] = df_persons["DEPL_OUI_NON"].fillna("2").astype("int")
    df_persons.loc[df_persons["DEPL_OUI_NON"] == 1, "travel_respondent"] = True
    df_persons.loc[df_persons["DEPL_OUI_NON"] == 2, "travel_respondent"] = False
    ...
    df_persons["trip_weight"] = df_persons["POND_PERS"]
    ...
    df_persons["socioprofessional_class"] = df_persons["PROFESSION"].fillna(8).astype(int)
