...
def execute(context):
    _, df_persons, df_activities = context.stage("hts")
    ...
    df_last["purpose"] = df_last["following_purpose"]

    df_last["start_time"] = df_last["arrival_time"]
    df_last["end_time"]   = np.nan

    df_last["activity_id"] = df_last["trip_id"] + 1
